...
wAmenities_good_id = a.add_amenities(Amenity.AIR_DRYER, Amenity.AUTOMATIC_SINK, Amenity.AUTOMATIC_TOILET)
wAmenities_bad_id = a.add_amenities(Amenity.LOTION, Amenity.CONTRACEPTION)
uPreferences_good_id = p.add_preference('women', False, False)
uPreferences_bad_id = p.add_preference('men', False, False)

# Create the building
good_building = b.add_building(Location(49.8105, -97.1335), 'GoodBuilding', '', 5, bRating_good_id)
bad_building = b.add_building(Location(49.8096, -97.1328), 'BadBuilding', '', 1, bRating_bad_id)
...
review_good_id = re.add_review(washroom_good_id, user_good_id, rRating_good_id, 'So amazing! Smelled like flowers.', 120)
review_bad_id = re.add_review(washroom_bad_id, user_bad_id, rRating_bad_id, 'SMELLS BAD!', 542)
