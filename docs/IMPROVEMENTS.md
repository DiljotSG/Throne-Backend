...
- [ ] Separate the database persistence logic to a separate server that handles DB update requests sequentially, doing so will eliminate possible concurrency issues when doing DB requests directly in the Lambda. Reading from the DB doesn't create any race conditions so the reading logic can remain in the Lambda without any concern.
